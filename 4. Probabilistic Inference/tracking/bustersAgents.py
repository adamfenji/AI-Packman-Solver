...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #get belief distributions for each living ghost
        livingGhosts = gameState.getLivingGhosts()
        livingGhostPositionDistributions = [
            beliefs for i, beliefs in enumerate(self.ghostBeliefs) if livingGhosts[i+1]
        ]

        #get most likely position for each ghost that is still alive
        mostLikelyPositions = []
        for d in livingGhostPositionDistributions:
            mostLikelyPositions.append(d.argMax())

        #get closest ghost by maze distance
        closestGhostDistance = 9999999999
        closestGhostPosition = None
        for position in mostLikelyPositions:
            distance = self.distancer.getDistance(gameState.getPacmanPosition(), position)
            if distance < closestGhostDistance:
                closestGhostDistance = distance
                closestGhostPosition = position

        #get action that minimizes the distance to the closest ghost
        bestAction = None
        shortestDistance = 9999999999
        for action in gameState.getLegalPacmanActions():
            distance = self.distancer.getDistance(Actions.getSuccessor(gameState.getPacmanPosition(), action), closestGhostPosition)
